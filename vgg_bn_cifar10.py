...
os.makedirs('figures/vgg_bn', exist_ok=True)
...
class VGG16_BN(nn.Module):
    """
    VGG16 architecture with Batch Normalization for CIFAR-10.
    
    The network contains 13 convolutional layers, each followed by a batch
    normalization layer, and 3 fully connected layers. This improves training
    stability and model performance compared to the standard VGG16.
    ...
    def forward(self, x):
        """
        Forward pass of the VGG16-BN model.
    ...
    def _initialize_weights(self):
        ...
        Uses Kaiming initialization for convolutional layers, constant
        initialization for batch normalization layers, and normal distribution
        for linear layers.
        """
...
def visualize_feature_maps(model, dataloader, filename='vgg_bn_feature_maps.png'):
    """
    ...
    plt.tight_layout()
    plt.savefig(f'figures/vgg_bn/{filename}')
    plt.close()

def plot_training_history(history, filename='vgg_bn_training_history.png'):
    """
    ...
    plt.grid(True)
    plt.savefig(f'figures/vgg_bn/vgg_bn_time_per_epoch.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    print("Creating VGG16 with Batch Normalization model...")
    vgg16_bn = VGG16_BN(num_classes=10)
    total_params = sum(p.numel() for p in vgg16_bn.parameters())
    print(f"VGG16_BN Total parameters: {total_params:,}")
    
    print("Training VGG16_BN model...")
    vgg16_bn, history = train_model(vgg16_bn, trainloader, epochs=50)
    
    print("Evaluating VGG16_BN on test data...")
    test_loss, test_acc = evaluate_model(vgg16_bn, testloader)
    print(f"VGG16_BN Test Accuracy: {test_acc:.2f}%")
    print(f"VGG16_BN Test Loss: {test_loss:.4f}")
    
    # Print average time per epoch
    avg_time = sum(history['time_per_epoch']) / len(history['time_per_epoch'])
    print(f"VGG16_BN Average time per epoch: {avg_time:.2f} seconds")
    
    # Save model
    torch.save(vgg16_bn.state_dict(), 'vgg16_bn_cifar10.pth')
    
    # Visualizations
    print("Generating visualizations...")
    visualize_feature_maps(vgg16_bn, testloader)
    ...
    with open('vgg16_bn_history.json', 'w') as f:
        json.dump(metrics, f)
    print("Metrics saved to vgg16_bn_history.json")
    
    print("VGG16_BN analysis completed!")
