...
os.makedirs('figures/alexnet', exist_ok=True)
...
class AlexNet(nn.Module):
    """
    Modified AlexNet architecture adapted for CIFAR-10.
    
    The network contains 5 convolutional layers and 3 fully connected layers,
    but with adjusted filter sizes and strides to accommodate the 32x32 input
    images of CIFAR-10 instead of the original 224x224 ImageNet images.
    ...
    def __init__(self, num_classes=10):
        ...
        # Modified AlexNet for CIFAR-10 (32x32 images instead of 224x224)
...
def visualize_filters(model, layer_idx=0, filename='alexnet_filters.png'):
    """
    ...
    plt.tight_layout()
    plt.savefig(f'figures/alexnet/{filename}')
    plt.close()

def visualize_feature_maps(model, dataloader, filename='alexnet_feature_maps.png'):
    """
...
def plot_training_history(history, filename='alexnet_training_history.png'):
    """
    ...
    plt.grid(True)
    plt.savefig(f'figures/alexnet/alexnet_time_per_epoch.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    alexnet, history = train_model(alexnet, trainloader, epochs=50)
    
    print("Evaluating AlexNet on test data...")
    test_loss, test_acc = evaluate_model(alexnet, testloader)
    ...
    print(f"AlexNet Average time per epoch: {avg_time:.2f} seconds")
    ...
    visualize_filters(alexnet)
    visualize_feature_maps(alexnet, testloader)
    ...
    with open('alexnet_history.json', 'w') as f:
        json.dump(metrics, f)
    print("Metrics saved to alexnet_history.json")
    
    print("AlexNet analysis completed!")
